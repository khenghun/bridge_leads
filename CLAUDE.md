...
- `app/common/querylog.py` — one SQLite row per simulation: timestamp, tool, request body. Nothing else — no IP, no session, no response. **Off unless `BRIDGE_QUERY_LOG` names a file**, so dev and tests write nothing; prod sets it in `docker-compose.prod.yml` against a bind-mounted `./data`. Both simulate routes call it *before* running, so an infeasible query (422) is logged too. Every write is best-effort and swallows its own errors — a full disk must cost you the log, not the app. Routes read it through the module (`querylog.QUERY_LOG`) so tests can swap the singleton.
