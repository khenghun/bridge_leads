...
- **Three batch caps, three batch functions, all in `dds_runtime`.** `analyse_all_plays` takes 20 plays per call (`analyse_plays`). `solve_all_boards` multithreads internally but its board array caps at `MAXNOOFBOARDS` (200); `calc_all_tables` caps at `MAXNOOFTABLES` (40). Passing more **raises**, which falls back to a single-board loop (~5x slower). Use `dds_runtime.solve_all()` / `dds_runtime.calc_tables()`; never call the endplay functions directly.
