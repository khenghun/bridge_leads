...
backend/worker/            the play solver's AWS Lambda worker handler (v1.5; Dockerfile target `worker`)
...
- `backend/engine/play/grader.py` — one Monte-Carlo grader parameterised by the **view** seat. `grade_position` samples the two hands that seat cannot see (declarer sees dummy; a defender sees dummy except at the opening lead), consistent with the play so far — cards an unseen seat already played are pinned via `fixed_cards`, a seat that showed out has that suit capped — replays each sample to the position and solves all samples in one `dds_runtime.solve_all` batch (`solve_board` prices every legal card of the player on play at once). `grade_play` walks the play and grades every decision of one seat (declarer's include dummy's cards); single-legal-card positions are `forced`, not solved. Method `double_dummy` solves the actual deal instead of sampling. Tricks are reported for the graded side; status thresholds on `diff = actual − best`: optimal (<0.1), good (≥ −0.3), else suboptimal. **Every graded decision carries `sample`** (play v1.4, `docs/play/v1.4-sample-size-plan.md`): the standard error of the per-deal paired difference *played − best* in tricks and IMPs — free, since `_solve` already returns each card's tricks per board (`per_board`, index-aligned across cards) — and `firm`, whether every corner of the ±2σ band classifies as the point estimate (`is_firm`); `summary.marginal` counts the rest. With `escalation=EscalationSettings(factor=3)` (the API default; `escalation: null` turns it off) a decision **in doubt** — status not optimal, or not firm — is re-graded on `factor × num_deals` (capped 600 plain / 300 expert) by **extending** the base sample: the extra layouts come from a per-decision stream (`_stream_rng(seed, index, tag)`), so un-escalated decisions and every base draw are bit-identical to a run without escalation; the expert filter's inner cap stays at the base count (`base_deals`), so earlier verdicts are memo hits and the population is unchanged, and `expert_layouts(start=...)` leads with the accepted layouts unexamined. `_carry_forward` pools are capped at the next decision's `n` before examination. Measured 2026-09-08: the per-deal se predicts the seed-to-seed spread (0.12 vs 0.10 at 100 deals), and no deal count makes a status firm whose truth sits between two lines — the band is the honest output. A third trigger (the unfiltered grade as a second opinion under expert opponents) was built, benchmarked and **dropped**: every extension it caused ended firm optimal at ~70 % of the added strict time — do not re-propose.
- `backend/engine/play/expert.py` — **Expert opponents** (play v1.3), a layout filter the grader applies when `expert=ExpertSettings(...)` is given: each sampled layout must be one on which every earlier opponent play was a best play *given what that opponent could see*, judged by a recursive single-dummy Monte-Carlo from the opponent's view. Cost control, in order: a **double-dummy trace** of the real line on the layout (`dds_runtime.analyse_plays`, 200 plays per DDS call, values = declarer tricks after each card) accepts every card that lost nothing DD — only *suspects* get the inner judgement (`strict=True` judges all); the verdict is **memoised on `(deal, j, view, holding)`** in a process-level `VerdictMemo` (survives across requests, seeded per key so it is deterministic); the inner sample is **sequential** (a geometric batch schedule from `max(5, DDS threads)`, stop when the paired-difference bound settles); cards adjacent in rank are one choice, and the opening lead is never judged (ruled 2026-09-04: the weakest inference at the highest price). The verdict compares **per-card means over common inner deals** — never a per-deal `best − actual`, which would call a coin-flip guess a blunder. Threshold = `tolerance + confidence·σ/√M` with σ ≈ 0.6 measured. The trace is the dominant non-strict cost (~13 ms/layout at 200 plays per call), so the outer loop draws only the deficit each round. The memo key's `holding` is the **judging** seat's remaining cards — declarer's for a play from dummy (v1.3 keyed those on dummy's public cards and reused one layout's verdict for all; fixed in v1.4). The outer loop starts from the layouts the same seat's previous decision accepted (carried forward via `VerdictMemo.pool_before`, kept only if still consistent with the cards played since — a uniform sample filtered is a uniform sample of the smaller set), so only the plays since get judged. A judgement's inner sample is a `sampling.LayoutStream` drawn wave by wave (the first k layouts of a seeded stream are the same however many follow, so verdicts equal the eager sample's while a judgement that settles on its first wave never draws the rest). Within a round, `_resolve` settles layouts memo-first and judges pending suspects in priority order (rejection rate per second, learnt in `VerdictMemo`) — the accept/reject answer is order-independent, only the cost moves. `grade_play(decisions=[...])` grades a subset of play indices so the UI can fetch a slow analysis trick by trick. `scripts/measure_expert.py` reproduces the plan's measurements on the six example hands.
...
- **Three batch caps, three batch functions, all in `dds_runtime`.** `analyse_all_plays` (`analyse_plays`) and `solve_all_boards` share the `MAXNOOFBOARDS` (200) board array — v1.3 capped traces at 20 per call by mistake, and widening to 200 was measured identical and 1.5x faster; `solve_all_boards` multithreads internally; `calc_all_tables` caps at `MAXNOOFTABLES` (40). Passing more **raises**, which falls back to a single-board loop (~5x slower). Use `dds_runtime.solve_all()` / `dds_runtime.calc_tables()`; never call the endplay functions directly.
...
- **Play v1.5 (in progress): strict mode fans out to AWS Lambda.** Built so far (2026-09-11): `backend/worker/handler.py` (ops `echo`/`health`/`bench`, sha-checked), `engine/version.py` (`engine_sha`, a hash of the engine sources), the Dockerfile's `worker` target, and the `build-worker`/`deploy-worker` CI jobs that push to ECR through the OIDC role — so **build the API with `--target backend`**, a plain build yields the worker. The function `bridge-play-worker` exists and answers (2026-09-11); both account raises landed 2026-09-14, so it runs at 7076 MB (four vCPUs) with concurrency 1 000. Not built: the seam, the client. `docs/play/v1.5-lambda-strict-plan.md` is the design; its *Provisioning progress* table says what already exists in the AWS account. The seam is judgement-level: the play API on the VPS keeps the memo, carried pools and result cache, and sends each `_resolve` wave's memo misses to a worker Lambda (a `worker` target of the backend Dockerfile running `_judge_batch`), verdicts bit-identical because a verdict is a pure function of its memo key. Three rules that follow: the engine gets a `JudgeBackend` protocol and a `LocalBackend` only — the Lambda client lives in `app_play/`, so the engine stays free of cloud imports; the worker is pinned `BRIDGE_DDS_THREADS=4` so the inner schedule's first step (`max(5, threads)`) matches prod's, or verdicts drift; and **AWS identifiers (account id, ECR URI, role ARNs, keys) never go in the repo** — names only in docs, values in `/opt/bridge_play/.env` and CI secrets.
