...
- Engine constraint dict (passed to `simulate_opening_lead`) is **keyed by player letter**, not endplay `Player` objects: `{'hcp': {'S': (min,max)}, 'suit_length': {'S': {'H': (min,max)}}, 'shapes': {'S': [ {suit:(min,max)}, ... ]}, 'quality': {'S': {'H': 'good'}}, 'fixed_cards': {'N': ['SA', ...]}}`. Either HCP/length bound may be `None` for unbounded. Only the three non-leader seats are constrainable. The API accepts shapes as **text** (the mini-language) and `service.py` parses them to terms. `quality` accepts **at most one entry in total** across every seat and suit; more raises (→ 422), and the frontend enforces it by clearing the previous pick (`applyQuality` in `lib/bridge.ts`), so users never hit that error.
- **A seat can now attract more than one acceptor predicate** (a shape *and* a suit quality). `lead_simulator._build_known_and_constraints` collects them per seat and ANDs them via `_all_of` — don't go back to `acceptors[p] = predicate`, which silently drops one.
...
- **libdds is not re-entrant.** Its multi-board functions (`SolveAllBoards*`) share global state and must be entered by one caller at a time — but `/api/simulate` requests run concurrently in Starlette's threadpool. `_solve_all()` therefore holds a global `_DDS_LOCK` around the solve; concurrent requests queue at that point (each still gets the full DDS thread allowance). Don't call DDS from anywhere else without taking the same lock.
...
- **Determinism:** `generate_deal(..., rng=)` takes a `random.Random` instance. `simulate_opening_lead(..., seed=)` builds a private `random.Random(seed)` — reproducible and thread-safe. `service.run_simulation()` calls it with `seed=0` behind an in-process LRU+TTL cache keyed on the canonical request, so identical inputs reuse the cached result (same contract the old `@st.cache_data` provided). The cache is guarded by `_cache_lock` with dogpile protection (`_inflight`): simultaneous identical requests simulate once — the first thread computes, the rest wait on its `Event` (covered by `tests/test_service_concurrency.py`).
